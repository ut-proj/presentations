...
In my 20s, I built a hobbyist home recording studio. This was the early-to-mid-90s, so everything was analog, as the new digital tech was far beyond my means. 
{{% /note %}}
...
While at the conference, though, Andrew and I talked about live coding, my interest in using LFE/OTP along with Extempore, and if I remember correctly, he mentioned conversations that he and Joe Armstrong had about Extempore and Erlang. Later in chatting with members of the LFE community, we talked briefly about exploring sound synthesis libraries in LFE, but nothing materialized.

However, a few years ago, I picked up guitar again, and this year I've begun playing piano and keyboards. I'm studing music theory, and even taking some online music courses.

Then along comes this Erlang Solutions blog post ...
...
The path to here from there was mostly straight-forward: I wanted to do more than generate tones. I wanted to control synthsizers and run services ... in short, make music. I read everything I could on Erlang and music, retracing Joe Armstrong's steps from the mid-2000s up until a few short years ago. 
...
{{% note %}}
I added enough functionality that I could generate a demo song with 9 seperate tracks of synthesizer being played. All of those were generated by undertone. This was featured in the 2021 Lambda Days "teaser" video for my talk.
...
[//]: Speaker-Notes:
{{% note %}}
The demo I had originally planning on showing you would have been a presentation of the code that was used to create the music in the teaser video.

Honestly, though, that's pretty boring: some manually defined chord definitions, some elementary transofrmations of lists using map and fold, etc.
{{% /note %}}
...
After updating undertone for this and creating a demo, I presented to the DFW Beamers ... but, I still wasn't satisfied ...
{{% /note %}}
...
[//]: Speaker-Notes:
{{% note %}}
I had promised to show conference attendees something with generative music, so I created a new piece with new code, and that's what I'm going to show you right now!

So, we'll have dessert first, and then come back for the meat and veg of architecture and code ...
{{% /note %}}
...
To be clear: the hard work of sending MIDI messages, playing its own sounds, etc., is all done by Extempore -- Andrew Sorensen's open source live-coding software. 
...
[//]: Speaker-Notes:
{{% note %}}
It runs a TCP server to which we can connect, and then from there, gain access to the operating system's audio layer, etc.
{{% /note %}}
...
[//]: Speaker-Notes:
{{% note %}}
undertone spawns Extmpore as a managed OS process, starts a TCP client, optionally Open Sound Control clients and even servers, and stuffs all this into a supervision tree that will restart the components, should you push them beyond their limits.
